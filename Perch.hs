...
newtype PerchM a= Perch{build :: Node -> Eff (dom :: DOM | eff) Node} deriving Typeable
...
instance monadPerch :: Monad PerchM where
   (>>) x y= mappend (unsafeCoerce x) y
   (>>=) = error "bind (>>=) invocation in the Perch monad creating DOM elements"
   return  = mempty

instance monadIOPerch :: MonadIO PerchM where
  liftIO mx= Perch $ \e -> mx >> return e

instance isStringPerch :: IsString Perch where
  fromString= toElem

class ToElem a where
  toElem :: a -> Perch

instance toElemString :: ToElem String where
   toElem s= Perch $ \e -> do
        e' <- newTextElem s
        addChild e' e
        return e'

instance   showa :: ToElem a  where toElem = toElem . show

instance toElema :: ToElem (PerchM a) where toElem e = unsafeCoerce e

attr tag (Tuple n v)=Perch $ \e -> do
        tag' <- build tag e
        setAttr tag' n v
        return tag'
...
child :: forall a. (ToElem a) => Perch -> a -> Perch
...
instance attributablePerchFun ::  Attributable (a -> Perch)  where
 (!) pe atrib = \e -> pe e `attr` atrib

instance attributablePerch :: Attributable Perch where
 (!) = attr
